...
@section('title', 'Field Groups - Shri-ms')
...
<div>
    ...
    <table class="w-full border-separate border-spacing-y-[15px] min-w-[1000px]">
        <thead> 
        <tr> 
            <th class="leading-4 text-gray-500 text-left font-normal text-[14px] dark:text-gray-dark-500 pl-5 w-3/5">Title</th>
        ...
        <tr>
            ...
            <div class="flex flex-col gap-y-2">
                <h3 class="leading-4 font-medium text-gray-1100 text-[16px] dark:text-gray-dark-1100">{{ $group->title }}</h3>
            </div>
            </td>
            <td class="border-y border-neutral dark:border-dark-neutral-bg">
                <span class="py-1 bg-neutral px-[10px] dark:bg-dark-neutral-border rounded-[5px] text-[#37404E] dark:text-[#b3b3b3] capitalize">{{ $group->is_active ? 'Active' : 'Inactive' }}</span>
            </td>
            <td class="border-y border-neutral border-r dark:border-dark-neutral-bg rounded-r-[7px]">
            <div class="flex gap-2">
                <a href="{{ route('cms.field-groups.edit', $group->id) }}" class="bg-neutral-bg mt-auto border-transparent rounded-lg transition-all duration-200 group dark:bg-dark-neutral-bg border-[4px] dark:hover:border-dark-neutral-border hover:border-neutral">
                ...
                <form action="{{ route('cms.field-groups.destroy', $group->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this field group?');">
                    @csrf
                    @method('DELETE')
        ...
        @empty
        <tr>
            <td colspan="3" class="py-5 text-center text-gray-500">No field groups found.</td>
        </tr>
        @endforelse
        </tbody>
    </table>
