    ...
    <table class="w-full border-separate border-spacing-y-[15px] min-w-[1000px]">
        ...
        <tr>
            ...
            <div class="flex gap-2">
                <a href="{{ route('cms.posts.edit', $post->id) }}" class="bg-neutral-bg mt-auto border-transparent rounded-lg transition-all duration-200 group dark:bg-dark-neutral-bg border-[4px] dark:hover:border-dark-neutral-border hover:border-neutral">
                ...
                <form action="{{ route('cms.posts.destroy', $post->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to trash this post?');">
                    @csrf
                    @method('DELETE')
        ...
        @empty
        <tr>
            <td colspan="5" class="py-5 text-center text-gray-500">No posts found.</td>
        </tr>
        @endforelse
        </tbody>
    </table>
