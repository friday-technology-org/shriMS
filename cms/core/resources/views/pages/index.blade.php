...
<div>
    ...
    <div class="flex items-center w-full"><img src="{{ asset('assets/images/icons/icon-search-normal.svg') }}" alt="search icon">
        <input name="search" value="{{ $search ?? '' }}" class="input w-full bg-transparent outline-none h-5 text-gray-800 text-sm leading-4 focus:!outline-none placeholder:text-gray-400 dark:text-white dark:placeholder:text-gray-dark-400 pl-[11px]" type="text" placeholder="Search page">
    </div>
    ...
    <table class="w-full border-separate border-spacing-y-[15px] min-w-[1000px]">
        ...
        <tr>
            ...
            <div class="flex flex-col gap-y-2">
                <h3 class="leading-4 font-medium text-gray-1100 text-[16px] dark:text-gray-dark-1100">{{ $page->title }}</h3>
                <span class="text-xs text-gray-400 dark:text-gray-dark-400">{{ $page->slug }}</span>
            </div>
            </td>
            <td class="border-y border-neutral dark:border-dark-neutral-bg">
            <div class="flex items-center gap-x-3">
                <img src="{{ $page->author?->avatar_url ?? asset('assets/images/cms-avt-1.png') }}" class="w-8 h-8 rounded-full" alt="avatar">
                <div class="flex flex-col gap-y-[7px]">
                <h4 class="leading-4 text-gray-1100 text-[14px] dark:text-gray-dark-1100">{{ $page->author->name ?? 'Unknown' }}</h4>
                </div>
            </div>
            </td>
            <td class="border-y border-neutral dark:border-dark-neutral-bg">
                <span class="py-1 bg-neutral px-[10px] dark:bg-dark-neutral-border rounded-[5px] text-[#37404E] dark:text-[#b3b3b3] capitalize">{{ $page->status }}</span>
            </td>
            <td class="border-y border-neutral dark:border-dark-neutral-bg"><span class="leading-4 text-gray-1100 text-[14px] dark:text-gray-dark-1100">{{ $page->published_at ? $page->published_at->format('d M Y') : '-' }}</span></td>
            <td class="border-y border-neutral border-r dark:border-dark-neutral-bg rounded-r-[7px]">
            <div class="flex gap-2">
                <a href="{{ $page->permalink }}" target="_blank" rel="noopener" class="bg-neutral-bg mt-auto border-transparent rounded-lg transition-all duration-200 group dark:bg-dark-neutral-bg border-[4px] dark:hover:border-dark-neutral-border hover:border-neutral">
                ...
                <a href="{{ route('cms.pages.edit', $page->id) }}" class="bg-neutral-bg mt-auto border-transparent rounded-lg transition-all duration-200 group dark:bg-dark-neutral-bg border-[4px] dark:hover:border-dark-neutral-border hover:border-neutral">
                ...
                <form action="{{ route('cms.pages.destroy', $page->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to trash this page?');">
                    @csrf
                    @method('DELETE')
        ...
        @empty
        <tr>
            <td colspan="5" class="py-5 text-center text-gray-500">No pages found.</td>
        </tr>
        @endforelse
        </tbody>
    </table>
