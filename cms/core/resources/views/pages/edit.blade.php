...
<form action="{{ route('cms.pages.update', $page->id) }}" method="POST">
    ...
    <div>
        ...
        <div class="flex justify-between gap-x-5 flex-col xl:flex-row">
            ...
            <div class="xl:w-[70%]">
            <div class="mb-6">
                <p class="text-gray-1100 text-base leading-4 font-medium capitalize mb-[10px] dark:text-gray-dark-1100">Page title</p>
                <div class="input-group border rounded-lg border-[#E8EDF2] dark:border-[#313442] sm:min-w-[252px]">
                    <input name="title" class="input w-full bg-transparent text-sm leading-4 text-gray-800 dark:text-white h-fit min-h-fit py-4 focus:outline-none pl-[13px] placeholder:text-inherit" type="text" placeholder="Add title" value="{{ old('title', $page->title) }}" required>
                </div>
            </div>
            ...
            <div class="mb-12">
                ...
                <div class="input-group border rounded-lg border-[#E8EDF2] dark:border-[#313442] sm:min-w-[252px]">
                    <input name="slug" class="input w-full bg-transparent text-sm leading-4 text-gray-800 dark:text-white h-fit min-h-fit py-4 focus:outline-none pl-[13px] placeholder:text-inherit" type="text" placeholder="page-slug" value="{{ old('slug', $page->slug) }}">
                </div>
            </div>
            
            <div class="mb-12">
                <p class="text-gray-1100 text-base leading-4 font-medium capitalize mb-[10px] dark:text-gray-dark-1100">Content</p>
                @include('cms-core::layouts.partials.wysiwyg-editor', ['name' => 'content', 'fieldId' => 'content-editor', 'value' => old('content', $page->content)])
            </div>
            
            <div class="mb-12"> 
                <p class="text-gray-1100 text-base leading-4 font-medium capitalize mb-[10px] dark:text-gray-dark-1100">Excerpt</p>
                <textarea name="excerpt" class="textarea w-full text-gray-800 dark:text-white resize-none rounded-lg bg-transparent border border-[#E8EDF2] dark:border-[#313442] p-4 min-h-[131px] focus:outline-none placeholder:text-inherit" placeholder="The excerpt">{{ old('excerpt', $page->excerpt) }}</textarea>
            </div>
                ...
                <div class="px-5 mb-5">
                    ...
                    <div class="input-group border rounded-lg border-[#E8EDF2] dark:border-[#313442] w-full">
                        <select name="status" id="status" class="select w-full bg-transparent text-sm leading-4 text-gray-800 dark:text-white h-fit min-h-fit py-3 focus:outline-none px-[13px]">
                            <option value="draft" class="bg-white dark:bg-dark-neutral-bg" {{ old('status', $page->status) == 'draft' ? 'selected' : '' }}>Draft</option>
                            <option value="published" class="bg-white dark:bg-dark-neutral-bg" {{ old('status', $page->status) == 'published' ? 'selected' : '' }}>Published</option>
                        </select>
                    </div>
                </div>
            ...
            @php
                ...
                $currentTemplate = $page->getMeta('_cms_page_template');
            @endphp
                    ...
                    <div class="input-group border rounded-lg border-[#E8EDF2] dark:border-[#313442] w-full">
                        <select name="meta[_cms_page_template]" class="select w-full bg-transparent text-sm leading-4 text-gray-800 dark:text-white h-fit min-h-fit py-3 focus:outline-none px-[13px]">
                            ...
                            @foreach($templates as $view => $name)
                                <option value="{{ $view }}" class="bg-white dark:bg-dark-neutral-bg" {{ $currentTemplate === $view ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="border border-neutral rounded-lg bg-neutral-bg dark:border-dark-neutral-border dark:bg-dark-neutral-bg overflow-hidden"
                 x-data="featuredImage({{ $page->featured_image_id ?? 'null' }}, '{{ $page->featuredImage?->thumbnailUrl('medium') ?? '' }}')">
